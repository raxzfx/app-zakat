<x-layout>
    <div class="p-4">
        <h1 class="uppercase text-xl">master data jenis penerimaan</h1>
        ...
        <div class="flex items-center justify-between mt-4 ">
            ...
            <div class="flex items-center">
                ...
                <a href="{{ route('jenis-penerimaan.create') }}" class="bg-biru text-white py-1 px-3 rounded-md text-sm transition-all duration-150 ease-in-out hover:bg-sky-700">
        ...
        <div class="flex items-center justify-between mt-4 text-sm">
            ...
            <form action="{{ route('jenis-penerimaan.index') }}" method="GET" class="relative mt-1 flex items-center">
    ...
    <button 
        ...
        <div class="overflow-x-auto w-full mt-6 mb-1">
            <table class="w-full bg-white border border-gray-200">
                ...
                <tbody>
                    @foreach ($jenisPenerimaan as $index => $jenis_penerimaan)
                        <tr>
                            <td class="px-6 py-4 border-b border-gray-200 bg-white text-sm">
                                {{ $jenisPenerimaan->firstItem() + $index }} </td>
                            <td class="px-6 py-4 border-b border-gray-200 bg-white text-sm">{{ $jenis_penerimaan->kode_jenis }}</td>
                            <td class="px-6 py-4 border-b border-gray-200 bg-white text-sm ">{{ $jenis_penerimaan->deskripsi }}</td>
                            <td class="px-6 py-4 border-b border-gray-200 bg-white text-sm">
                                <!-- button edit -->
                                <a href="{{ route('jenis-penerimaan.edit', $jenis_penerimaan->kode_jenis) }}"
                                ...
                                <!-- a delete -->
                                <form action="{{ route('jenis-penerimaan.destroy', $jenis_penerimaan->kode_jenis) }}" method="POST" class="mt-2"
